...
class AccountSync:
    ...
    def user_sync(self) -> float:
        try:
            start_time = time.time()
            users = User.objects.all().values(
                'id', 'username', 'password', 'fio', 'is_active',
                'created_at', 'updated_at', 'departmant', 'position', 'room')
            ids = []
            for i in users:
                ids.append(i['id'])
            ...
            User_offline.objects.exclude(id__in=ids).delete()
            stop_time = time.time()
            return stop_time - start_time
        except Exception as e:
            logger.error('user_sync' + str(e))
            raise e
