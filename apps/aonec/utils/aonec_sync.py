...
class OneCFullSync:
    ...
    def onec_ttn_full_sync(self) -> float:
        try:
            ...
            onec_ttn = OneCTTN.objects.all().order_by('id')
            list_ttn = []
    ...
    def onec_ttn_item_full_sync(self) -> float:
        try:
            ...
            onec_ttn_items = OneCTTNItem.objects.select_related('onec_ttn').all().order_by('id')
            list_ttn_item = []
...
class OneCSync:
    ...
    def onec_ttn_sync(self) -> float:
        try:
            ...
            onec_tnn = OneCTTN.objects.filter(id__gt=last_onec_ttn.id if last_onec_ttn else 0)
    ...
    def onec_ttn_item_sync(self) -> float:
        try:
            ...
            onec_ttn_item = OneCTTNItem.objects.filter(id__gt=last_onec_ttn_item.id if last_onec_ttn_item else 0)
            ...
            for i in onec_ttn_item.iterator(chunk_size=self.batch_size):
                ...
                if len(list_items) >= self.batch_size:
